...
func (a *App) seed() {
	...
	if c > 0 {
		...
		a.ensureVariableQRCommands(org)
		return
	}
...
func (a *App) ensureVariableQRCommands(org string) {
	placeIDs := a.ids("select id from places where organization_id=? and active=1 order by id", org)
	productIDs := a.ids("select id from products where organization_id=? and active=1 order by id", org)
	for _, pl := range placeIDs {
		for _, pr := range productIDs {
			for _, act := range []string{"add", "subtract"} {
				if _, err := a.db.Exec("insert or ignore into qr_commands(id,organization_id,place_id,product_id,action,amount,token,active)values(?,?,?,?,?,?,?,1)", id(), org, pl, pr, act, 0, "amt-"+id()); err != nil {
					log.Printf("variable qr command failed: %v", err)
				}
			}
		}
	}
...
var germanMessages = map[string]string{
	...
	"login.dev_operator":      "Als Dev-Operator anmelden",
	"login.dev_admin":         "Als Dev-Admin anmelden",
	...
	"scan.copy":               "Erlaube den Kamerazugriff und richte den QR-Code im Rahmen aus. Zest öffnet gültige Scan-Links automatisch.",
	"scan.starting":           "Kamera wird gestartet …",
	"scan.start_button":       "Kamera starten",
	"scan.cancel":             "Schließen",
	"scan.unavailable":        "Kamera nicht verfügbar",
	"scan.denied":             "Kamerazugriff verweigert",
	"scan.none":               "Noch kein QR-Code erkannt",
	"scan.failed":             "Scan fehlgeschlagen, bitte erneut versuchen",
	"scan.insecure":           "Kamera nur über HTTPS oder localhost verfügbar",
	"scan.ready":              "Scannen …",
	"scan.found":              "QR-Code gefunden. Öffne Scan …",
	"scan.unsupported":        "QR-Erkennung ist nicht verfügbar. Lade die Seite neu oder nutze die Systemkamera deines Geräts.",
	"scan.camera_error":       "Kamera konnte nicht gestartet werden. Prüfe die Browserberechtigungen und lade die Seite neu.",
	...
	"manual.amount":           "Menge",
	"amountqr.eyebrow":        "QR-Mengenbuchung",
	"amountqr.manual_amount":  "Menge eingeben",
	"amountqr.print_kind":     "Bestand buchen · Menge eingeben",
	...
	"devqr.general":           "Allgemeine Token-QR-Codes",
	"devqr.invite":            "Einladung",
	"devqr.place":             "Ortslink",
	...
	"result.scan_copy":        "Scanne den nächsten QR-Code direkt in Zest.",
	"result.scan_later":       "Die Kamera öffnet automatisch gültige Zest-QR-Links.",
}
...
var englishMessages = map[string]string{
	...
	"login.dev_operator":      "Log in as dev operator",
	"login.dev_admin":         "Log in as dev admin",
	...
	"scan.copy":               "Allow camera access and point the QR code inside the frame. Zest opens valid scan links automatically.",
	"scan.starting":           "Starting camera…",
	"scan.start_button":       "Start camera",
	"scan.cancel":             "Close",
	"scan.unavailable":        "Camera unavailable",
	"scan.denied":             "Camera permission denied",
	"scan.none":               "No QR code detected yet",
	"scan.failed":             "Scan failed, try again",
	"scan.insecure":           "Camera requires HTTPS or localhost",
	"scan.ready":              "Scanning…",
	"scan.found":              "QR code found. Opening scan…",
	"scan.unsupported":        "QR detection is not available. Reload the page or use your device camera instead.",
	"scan.camera_error":       "Could not start the camera. Check browser permissions and reload the page.",
	...
	"amountqr.eyebrow":        "QR amount entry",
	"amountqr.manual_amount":  "Enter amount",
	"amountqr.print_kind":     "Book stock · enter amount",
	"manual.add":              "Add",
	...
	"devqr.general":           "General token QR codes",
	"devqr.invite":            "Invite",
	"devqr.place":             "Place link",
	...
	"result.scan_copy":        "Scan the next QR code directly in Zest.",
	"result.scan_later":       "The camera opens valid Zest QR links automatically.",
}
...
func (a *App) routes(m *http.ServeMux) {
	...
	m.Handle("/static/", http.StripPrefix("/static/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	m.HandleFunc("/dev/login", a.devLogin)
	...
	m.HandleFunc("/scan/amount/", a.amountScan)
	...
	m.HandleFunc("/admin", func(w http.ResponseWriter, r *http.Request) {
		if c, ok := a.admin(w, r); ok {
			a.render(w, r, "admin", a.adminDashboardMetrics(c.OrgID))
		}
	})
}
...
func (a *App) login(w http.ResponseWriter, r *http.Request) {
	...
	a.render(w, r, "login", loginData{ReturnPath: sanitizeReturnPath(r.URL.Query().Get("return")), DevLogin: a.env != "production"})
}
...
func (a *App) devLogin(w http.ResponseWriter, r *http.Request) {
	...
	var exists int
	if err := a.db.QueryRow("select count(*) from users where id=?", uid).Scan(&exists); err != nil || exists != 1 {
		http.Error(w, "development user is not seeded", http.StatusServiceUnavailable)
		return
	}
	if err := a.createSession(w, uid); err != nil {
		http.Error(w, "could not create development session", 500)
		return
	}
	http.Redirect(w, r, sanitizeReturnPath(r.URL.Query().Get("return")), http.StatusFound)
}
...
func (a *App) authLogin(w http.ResponseWriter, r *http.Request) {
	if a.auth0.ClientID == "" || a.auth0.Issuer == "" {
		if a.env != "production" {
			http.Redirect(w, r, "/dev/login?return="+url.QueryEscape(sanitizeReturnPath(r.URL.Query().Get("return"))), http.StatusFound)
			return
		}
...
type amountScanData struct {
	Token, Place, Product, Unit, Color, Action, ActionLabel string
}

func (a *App) amountScan(w http.ResponseWriter, r *http.Request) {
	c, ok := a.approved(w, r)
	if !ok {
		return
	}
	token := strings.TrimPrefix(r.URL.Path, "/scan/amount/")
	var org, pl, pr, act, pn, unit, color, place string
	var pactive, practive, qactive int
	err := a.db.QueryRow(`select q.organization_id,q.place_id,q.product_id,q.action,q.active,p.active,pr.active,p.name,pr.name,pr.unit,coalesce(pr.color,'') from qr_commands q join places p on p.id=q.place_id join products pr on pr.id=q.product_id where q.token=? and q.amount=0`, token).Scan(&org, &pl, &pr, &act, &qactive, &pactive, &practive, &place, &pn, &unit, &color)
	...
	if r.Method == "POST" {
		...
		eid := id()
		if _, err := a.db.Exec("insert into inventory_events(id,organization_id,place_id,product_id,user_id,quantity_delta,event_type,note)values(?,?,?,?,?,?,?,?)", eid, org, pl, pr, c.UserID, delta, act, "QR amount entry"); err != nil {
	...
	a.render(w, r, "amount_scan", amountScanData{Token: token, Place: place, Product: pn, Unit: unit, Color: color, Action: act, ActionLabel: actionLabel(c, act)})
}
...
func (a *App) adminDashboardMetrics(org string) map[string]int {
	metrics := map[string]int{}
	metricQueries := map[string]string{
		"TotalStock":     `select coalesce(sum(quantity_delta),0) from inventory_events where organization_id=?`,
		"EventsToday":    `select count(*) from inventory_events where organization_id=? and date(created_at)=date('now')`,
		"Pending":        `select count(*) from memberships where organization_id=? and status='pending'`,
		"NegativeStock":  `select count(*) from (select place_id,product_id,sum(quantity_delta) stock from inventory_events where organization_id=? group by place_id,product_id having stock<0)`,
		"ActivePlaces":   `select count(*) from places where organization_id=? and active=1`,
		"ActiveProducts": `select count(*) from products where organization_id=? and active=1`,
	}
	for key, query := range metricQueries {
		var value int
		a.db.QueryRow(query, org).Scan(&value)
		metrics[key] = value
	}
	return metrics
}
...
func (a *App) adminPlacePost(w http.ResponseWriter, r *http.Request, c Ctx) {
	...
	if action == "create" || placeID == "" {
		...
		a.ensurePlaceQRCommands(c.OrgID, placeID)
		a.ensureVariableQRCommands(c.OrgID)
		http.Redirect(w, r, "/admin/places", 303)
		return
	}
...
func (a *App) adminPrintQRCodes(w http.ResponseWriter, r *http.Request, c Ctx) {
	...
	placeGroups := map[string]map[string]any{}
	...
	for rows.Next() {
		...
		} else if kind == "amount_command" && commandToken != "" {
			target = a.amountScanURL(commandToken)
		}
		if color == "" && (kind == "command" || kind == "amount_command") {
			color = defaultProductColor(product)
		}
		code := map[string]any{"ID": id, "Label": label, "Kind": kind, "Place": place, "Product": product, "Color": color, "Action": action, "ActionLabel": actionLabel(c, action), "Amount": amount, "Target": target, "Img": a.qrImageURL(target)}
		codes = append(codes, code)
		if place == "" {
			general = append(general, code)
			continue
		}
		if _, ok := placeGroups[place]; !ok {
			placeGroups[place] = map[string]any{"Name": place, "Codes": []map[string]any{}}
			placeOrder = append(placeOrder, place)
		}
		placeGroups[place]["Codes"] = append(placeGroups[place]["Codes"].([]map[string]any), code)
	}
	groups := []map[string]any{}
	if len(general) > 0 {
		groups = append(groups, map[string]any{"Name": tr(c, "devqr.general"), "Codes": general})
	}
	for _, place := range placeOrder {
		groups = append(groups, placeGroups[place])
	}
	md := a.manualData(c, "", "")
	a.render(w, r, "printqr", printQRData{Codes: codes, Groups: groups, Places: md.Places, Products: md.Products, Amounts: a.amounts})
}
...
func (a *App) adminPrintQRPost(w http.ResponseWriter, r *http.Request, c Ctx) {
	...
	if kind == "place" || kind == "command" || kind == "amount_command" {
		placeID = r.FormValue("place_id")
	...
	if kind == "command" || kind == "amount_command" {
		productID = r.FormValue("product_id")
		amount, _ := strconv.Atoi(r.FormValue("amount"))
		if kind == "amount_command" {
			amount = 0
		}
		act := r.FormValue("command_action")
		if (kind == "command" && amount <= 0) || (act != "add" && act != "subtract") || !a.activePlaceProduct(c.OrgID, placeID, productID) {
	...
	if kind != "home" && kind != "place" && kind != "command" && kind != "amount_command" {
...
func defaultPrintQRLabel(db *sql.DB, c Ctx, kind, placeID, productID, commandID string) string {
	...
	if kind == "amount_command" {
		db.QueryRow("select pl.name,pr.name,q.action from qr_commands q join places pl on pl.id=q.place_id join products pr on pr.id=q.product_id where q.id=?", commandID).Scan(&place, &product, &act)
		return fmt.Sprintf("%s · %s · %s", actionLabel(c, act), product, place)
	}
...
func (a *App) amountScanURL(token string) string {
	return strings.TrimRight(a.base, "/") + "/scan/amount/" + token
}
...
func (a *App) qrMatrix(w http.ResponseWriter, r *http.Request, c Ctx) {
	...
	for rows.Next() {
		...
		label := fmt.Sprintf("%s %d %s", actionLabel(c, act), amt, prod)
		if amt == 0 {
			scanURL = a.amountScanURL(tok)
			label = fmt.Sprintf("%s %s · %s", actionLabel(c, act), prod, tr(c, "amountqr.manual_amount"))
		}
		out = append(out, map[string]any{"Action": act, "Label": label, "Amount": amt, "Href": scanURL, "Img": a.qrImageURL(scanURL), "Color": color, "PlaceURL": placeURL, "PlaceImg": a.qrImageURL(placeURL)})
	}
...
func (a *App) devQRCodes(w http.ResponseWriter, r *http.Request) {
	...
	data := devQRData{}
	inviteRows, err := a.db.Query(`select token from organization_invites where organization_id=? and active=1 order by created_at`, c.OrgID)
	if err != nil {
		http.Error(w, "could not load invite QR codes", 500)
		return
	}
	for inviteRows.Next() {
		var tok string
		inviteRows.Scan(&tok)
		target := strings.TrimRight(a.base, "/") + "/org/join/" + tok
		data.General = append(data.General, map[string]any{"Label": tr(c, "devqr.invite"), "Href": "/org/join/" + tok, "Img": a.qrImageURL(target)})
	}
	inviteRows.Close()

	placeRows, err := a.db.Query(`select id,name,token from places where organization_id=? and active=1 order by name`, c.OrgID)
	if err != nil {
		http.Error(w, "could not load place QR buckets", 500)
		return
	}
	defer placeRows.Close()
	for placeRows.Next() {
		var placeID, place, placeToken string
		placeRows.Scan(&placeID, &place, &placeToken)
		placeTarget := strings.TrimRight(a.base, "/") + "/places/" + placeToken
		bucket := map[string]any{"Name": place, "Codes": []map[string]any{{"Place": place, "Label": tr(c, "devqr.place"), "Href": "/places/" + placeToken, "Img": a.qrImageURL(placeTarget), "Product": "", "Color": ""}}}
		rows, err := a.db.Query(`select pr.name,coalesce(pr.color,''),q.action,q.amount,q.token from qr_commands q join products pr on pr.id=q.product_id where q.organization_id=? and q.place_id=? and q.active=1 and pr.active=1 order by pr.name,q.action,q.amount`, c.OrgID, placeID)
		if err != nil {
			http.Error(w, "could not load QR commands", 500)
			return
		}
		codes := bucket["Codes"].([]map[string]any)
		for rows.Next() {
			var prod, color, act, tok string
			var amt int
			rows.Scan(&prod, &color, &act, &amt, &tok)
			href := "/scan/" + tok
			scanURL := a.scanURL(tok)
			if amt == 0 {
				href = "/scan/amount/" + tok
				scanURL = a.amountScanURL(tok)
			}
			codes = append(codes, map[string]any{"Place": place, "Product": prod, "Color": color, "Action": act, "ActionLabel": actionLabel(c, act), "Amount": amt, "Href": href, "Img": a.qrImageURL(scanURL)})
		}
		rows.Close()
		bucket["Codes"] = codes
		data.Places = append(data.Places, bucket)
	}
	a.render(w, r, "devqr", data)
}
...
{{define "scan"}}{{template "layout" .}}{{end}}
{{define "amount_scan"}}{{template "layout" .}}{{end}}
