...
func (a *App) places(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		var pid, n, t string
		rows.Scan(&pid, &n, &t)
		v = append(v, map[string]string{"Name": n, "Token": t, "Summary": a.placeStockSummary(c, pid)})
	}
	a.render(w, r, "places", v)
}

func (a *App) placeStockSummary(c Ctx, placeID string) string {
	...
	for rows.Next() {
		...
		parts = append(parts, fmt.Sprintf("%d %s %s", qty, unitLabel(c, unit), name))
	}
	if len(parts) == 0 {
		return tr(c, "places.cardcopy")
	}
