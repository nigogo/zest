...
func (a *App) qrMatrix(w http.ResponseWriter, r *http.Request, c Ctx) {
	...
	for rows.Next() {
		...
		out = append(out, map[string]any{"Action": act, "Label": fmt.Sprintf("%s %d %s", strings.Title(act), amt, prod), "Img": template.HTML(svg)})
	}
	a.render(w, r, "qr", out)
}
