...
func (a *App) placeStockOverview(c Ctx, placeID string) []map[string]any {
	rows, err := a.db.Query(`select p.name,p.unit,coalesce(sum(e.quantity_delta),0) as stock from products p left join inventory_events e on e.product_id=p.id and e.place_id=? where p.organization_id=? and p.active=1 group by p.id order by stock desc,p.name limit 3`, placeID, c.OrgID)
	if err != nil {
		return nil
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var qty int
		rows.Scan(&name, &unit, &qty)
		overview = append(overview, map[string]any{"Name": name, "Unit": unitLabel(c, unit), "Qty": qty})
	}
	return overview
}
...
const tpl = `{{define "layout"}}<!doctype html><html lang="{{.Ctx.Lang}}"><head><title>Zest · {{.Title}}</title><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="/static/app.css"><script src="https://unpkg.com/htmx.org@1.9.12"></script><script src="https://unpkg.com/hyperscript.org@0.9.12"></script></head><body><header class="mobile-header"><div><a class="wordmark" href="/places"><img src="/static/zest-logo.svg" alt="" width="32" height="32">Zest</a></div><nav class="header-nav" aria-label="Primary"><a class="button ghost icon-button" href="/places" title="{{t .Ctx "nav.places"}}"><svg class="nav-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M4 11.5 12 5l8 6.5V20h-5v-5H9v5H4z"/></svg></a><a class="button ghost icon-button" href="/events" title="{{t .Ctx "nav.events"}}"><svg class="nav-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 3h10v3h3v15H4V6h3zm2 2v3h6V5zm-1 7v2h8v-2zm0 4v2h8v-2z"/></svg></a><a class="button ghost icon-button" href="/settings" title="{{t .Ctx "nav.settings"}}"><svg class="nav-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M10.8 3h2.4l.5 3a6.8 6.8 0 0 1 1.4.6l2.5-1.8 1.7 1.7-1.8 2.5c.3.5.5.9.6 1.4l3 .5v2.4l-3 .5a6.8 6.8 0 0 1-.6 1.4l1.8 2.5-1.7 1.7-2.5-1.8c-.5.3-.9.5-1.4.6l-.5 3h-2.4l-.5-3a6.8 6.8 0 0 1-1.4-.6l-2.5 1.8-1.7-1.7 1.8-2.5a6.8 6.8 0 0 1-.6-1.4l-3-.5v-2.4l3-.5c.1-.5.3-.9.6-1.4L4.7 6.5l1.7-1.7 2.5 1.8c.5-.3.9-.5 1.4-.6zm1.2 7a2 2 0 1 0 0 4 2 2 0 0 0 0-4z"/></svg></a>{{if eq .Ctx.Role "admin"}}<a class="button ghost icon-button" href="/admin" title="{{t .Ctx "nav.admin"}}"><svg class="nav-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3 20 6v5c0 5-3.2 8.6-8 10-4.8-1.4-8-5-8-10V6z"/></svg></a>{{end}}{{if .Ctx.Authed}}<form method="post" action="/logout">{{csrf .Ctx}}<button class="ghost icon-button" title="{{t .Ctx "nav.logout"}}"><svg class="nav-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M10 5 4 11l6 6v-4h5a3 3 0 0 1 3 3v2h2v-2a5 5 0 0 0-5-5h-5z"/></svg></button></form>{{end}}</nav></header><main class="app-main">{{template "body" .}}</main></body></html>{{end}}
